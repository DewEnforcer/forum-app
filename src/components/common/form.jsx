...
class Form extends Component {
    ...
    renderSelect = ({path, name, label, type}, options) => {
        const {data, errors} = this.state;
    ...
    renderTextarea = ({path, name, label}) => {
        const {data, errors} = this.state;
        return <Textarea key={path} label={label} name={name} error={errors[path]} value={data[path]} onChange={this.handleChange}/>
    }
    renderButton = (label) => {
        return (
